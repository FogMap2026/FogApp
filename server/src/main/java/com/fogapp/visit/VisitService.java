...
public class VisitService {
    ...
    /**
     * 방문을 인증한다. userId 는 인증 필터가 세운 현재 사용자여야 하며 요청 본문에서 받지 않는다.
     *
     * <p>순서가 중요하다 — 스팟 존재 → 중복 → 반경 순으로 검사한다.
     * 반경을 먼저 보면 "없는 스팟"과 "멀리 있는 스팟"이 같은 응답으로 뭉개진다.</p>
     */
    @Transactional
    public Visit verify(Long userId, Long spotId, String photoUrl, double lat, double lng) {
