...
|------|------|
| **Phase 0 기반** | ✅ 완료 |
| **Phase 1 인증·데이터** | ✅ 완료 — `main`까지 승격 ([#25](../../pull/25)) |
| **Phase 2 지도 코어** | ✅ 완료 — 스택 PR 5건 전부 병합 |
| **Phase 2 소셜 트랙** | ✅ 완료 |
| **Phase 3 탐험 루프 ★MVP★** | 🔄 **6/8 완료** — 남은 것은 [#49](../../issues/49) 안개 걷힘·[#50](../../issues/50) 정보 해금 |
| **Phase 4 발자취 통합** | 🔄 **착수** — 이슈 [#70](../../issues/70)~[#73](../../issues/73) 등록·할당 완료 |
| **Phase 5~7** | ⬜ 미착수 |
...
| 3-4 인증 API + 사진 저장 | ✅ [#48](../../issues/48) |
| 3-5 안개 걷힘 | 🔄 [#49](../../issues/49) — [PR #74](../../pull/74) 리뷰 중 |
| 3-6 스팟 정보 해금 | ⬜ [#50](../../issues/50) |
...
1. **스팟 데이터가 없습니다** — `spots` 테이블이 비어 있어 **지도에 마커가 하나도 없고 정복률도 `--%`** 입니다.
   [#5](../../issues/5) 수집 배치를 한 번 돌려야 하며, TourAPI 키와 서버 구동이 필요합니다.
   실기기 확인에서 드러난 **실질적으로 가장 큰 병목**입니다.
2. **[#61](../../pull/61) CI 개선 미병합** — iOS 빌드 검증·Android APK 아티팩트·Flutter 3.44.9 고정이 여기 묶여 있습니다.
   병합 전까지 `dev`의 CI는 **네이티브 빌드를 검증하지 못합니다**(`flutter analyze`만 실행).
3. **[#76](../../issues/76) 사진 업로드 용량 상한 없음** — 사진 보관이 서버 디스크로 옮겨지면서
   무제한 업로드가 **서비스 중단 위험**이 됐습니다.
