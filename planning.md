...
| **Phase 3 탐험 루프 ★MVP★** | 🔄 진행 중 — 근접 감지([#45](../../issues/45))·인증 API([#48](../../issues/48))·Storage 규칙 병합 완료. 사진 촬영 UI([#47](../../pull/57))·정복률 API·UI([#51](../../issues/51))가 리뷰/병합 대기. 안개 걷힘 연출([#49](../../issues/49))·스팟 정보 해금([#50](../../issues/50))·인앱 알림([#46](../../issues/46))은 미착수 |
| **Phase 4~7** | ⬜ 미착수 |

**해소됨** — [#52](../../issues/52) 발자취·매칭 API가 클라이언트의 `userId`를 그대로 신뢰하던 문제는 [#53](../../pull/53)에서 `AuthUser` 기반으로 수정되어 `dev`에 병합되었습니다.

**지금의 병목**

1. **[#57](../../pull/57) 인증 사진 업로드 경로 불일치** — 업로드 코드가 `visit_photos/{uid}/{spotId}-{ts}.jpg` 경로를 쓰는데, [#56](../../pull/56)의 Storage 보안 규칙은 `visits/{userId}/{spotId}/{fileName}`을 기대합니다. 지금 그대로면 업로드가 전부 `PERMISSION_DENIED`로 실패합니다 — 경로를 규칙에 맞춰 고쳐야 리뷰 통과됩니다.
2. **[#61](../../pull/61) iOS 빌드 CI 실패** — `AppDelegate.swift`가 최신 Flutter의 "implicit engine" API(`FlutterImplicitEngineDelegate`/`FlutterImplicitEngineBridge`)로 스캐폴딩되어 있는데, 이 레포는 CI에 Flutter 3.24로 고정돼 있어 해당 API가 없습니다. 3.24 호환 형태(`GeneratedPluginRegistrant.register(with: self)`를 `didFinishLaunchingWithOptions`에서 직접 호출)로 되돌려야 합니다.
3. **[#51](../../issues/51) 정복률** — API([#60](../../pull/60))와 UI([#62](../../pull/62))가 각각 리뷰 통과했지만 둘 다 아직 `dev`에 병합 전입니다. UI는 base가 `dev`라 API가 먼저 병합되어야 실제로 동작합니다(머지 순서: #60 → #62).
...
| 3-3 | 현장 사진 촬영 → **방문 인증** UI | 🟩 UI | 송진오 | 🔄 리뷰에서 경로 불일치 지적, 수정 대기 | [#47](../../issues/47) ([PR #57](../../pull/57)) | `feat/ui-visit-verify` |
...
| 3-7 | 지역별 **정복률(%) 계산·표시** | 🟦 API / 🟩 UI | 박근호·송진오 | 🔄 리뷰 통과, `dev` 병합 대기 (API 먼저) | [#51](../../issues/51) ([PR #60](../../pull/60)·[#62](../../pull/62)) | `feat/api-conquest-rate`, `feat/ui-conquest` |
...
1. **[#57](../../pull/57) 사진 업로드 경로 수정** — Storage 규칙([#56](../../pull/56))과 실제 업로드 코드의 경로가 달라 리뷰에서 반려됨. 고치면 3-3이 닫힙니다.
2. **[#60](../../pull/60)·[#62](../../pull/62) 정복률 병합** — 둘 다 리뷰는 통과. `#60`(서버 API)을 먼저 `dev`에 병합해야 `#62`(클라이언트)가 실제로 동작합니다.
3. **[#49](../../issues/49) 안개 걷힘·[#50](../../issues/50) 정보 해금·[#46](../../issues/46) 근접 알림** — 아직 미착수. 3-4(인증 API)가 이미 열려 있으니 바로 시작 가능합니다.

**팀이 정한 것**

- **인증 반경** — 3-1(geofencing)·3-4(서버 검증)가 같은 값(100m 진입 / 130m 이탈, 히스테리시스)을 씁니다.
- **정복률 지역 단위** — **시/군/구**로 확정되어 [#60](../../pull/60)에 반영됐습니다.
- **3-2 알림 범위** — 아직 미정. 백그라운드 위치 추적은 배터리·스토어 심사 부담이 커서, 1차는 포그라운드+로컬 알림으로 좁히는 것을 제안합니다.
...
| **M3** | **탐험 핵심 루프 동작 (MVP)** · 🟪 매칭 알고리즘 동작 | `v0.4` | 🔄 매칭·근접감지·인증 API ✅ / 인증 UI·정복률 병합 대기 / 안개 걷힘·정보 해금·알림 ⬜ |
