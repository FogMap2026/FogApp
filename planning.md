...
| **Phase 3 탐험 루프 ★MVP★** | 🔄 진행 중 — 근접 감지([#45](../../issues/45))·인증 API([#48](../../issues/48))·Storage 규칙·정복률([#51](../../issues/51), [PR #60](../../pull/60)·[#62](../../pull/62)) 병합 완료. 사진 촬영 UI([#47](../../pull/57))는 리뷰에서 반려되어 수정 대기. 안개 걷힘 연출([#49](../../issues/49))·스팟 정보 해금([#50](../../issues/50))·인앱 알림([#46](../../issues/46))은 미착수 |
| **Phase 4~7** | ⬜ 미착수 |
...
2. **[#49](../../issues/49) 안개 걷힘·[#50](../../issues/50) 정보 해금·[#46](../../issues/46) 근접 알림** — 아직 미착수. 3-4(인증 API)가 이미 열려 있으니 바로 시작 가능합니다.
...
| **M3** | **탐험 핵심 루프 동작 (MVP)** · 🟪 매칭 알고리즘 동작 | `v0.4` | 🔄 매칭·근접감지·인증 API·정복률 ✅ / 인증 UI 수정 대기 / 안개 걷힘·정보 해금·인앱 알림 ⬜ |
