...
| **Phase 3 탐험 루프** | ⬜ 미착수 (선행 3-8 매칭 알고리즘만 ✅ 완료) |
| **Phase 4~7** | ⬜ 미착수 |
...
| 3-8 | 🟪 **매칭 알고리즘**(성향 유사도 + 지역/거리 필터) 선행 구현 | 🟪 SOC | ✅ | [#37](../../issues/37) ([PR #38](../../pull/38)) |

> 🟪 소셜 트랙: 3-8은 성향 데이터(2-6)에만 의존 — 탐험 루프와 무관하게 병렬. Phase 5에서 화면만 붙이면 됨.

> **현재**: 소셜 트랙(3-8)이 먼저 끝나 `GET /api/matches/candidates`가 동작합니다.
> 탐험 루프(3-1~3-7)는 Phase 2 지도 PR이 병합되어야 착수할 수 있어 **MVP 도달의 실질 선행조건은 [PR #34](../../pull/34) 병합**입니다.
> `visits` 테이블은 [#1](../../issues/1)에서 이미 만들어져 있어, 3-4 인증 API는 스키마 작업 없이 바로 시작할 수 있습니다.
