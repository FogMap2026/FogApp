...
///
/// 1. **짙은 안개**([_fog]) — 지도 이동 범위 전체를 덮는 사각형 한 장, [fogColor](85%
///    반투명). 대한민국 땅만이 아니라 바다·이웃 나라까지 전부 덮되, 아래 지도가 은은히
///    비쳐 «지도로서» 살아 있다 — 40m 시야 밖도 길·지명은 읽힌다.
/// 2. **내 시야**([_vision]) — 현재 위치 반경 [visionRadiusMeters](40m) 원. 짙은 안개에
///    이 원만큼 구멍을 내고, 그 자리에 [visionFogColor](짙은 안개보다 옅은 50%)를
///    덮는다 — «바로 곁은 확실히 더 잘 보인다».
...
class FogOverlayController {
  ...
  /// 짙은 안개 — 85% 반투명(예전 전역 안개와 같은 값). 아래 지도가 은은히 비치도록
  /// 완전 불투명에서 되돌렸다(PR #213 리뷰) — 완전히 가리면 40m 시야 밖은 «지도가
  /// 아니게» 된다. 도로·지명이 안 보이면 스팟까지 어떻게 가는지 알 수 없다.
  static const fogColor = Color(0xD948566B);

  /// 내 시야의 반투명 안개 — 짙은 안개(85%)보다 옅게(50%) 잡아 «곁은 확실히 더
  /// 잘 보인다»를 유지한다.
  static const visionFogColor = Color(0x8048566B);

  /// 짙은 안개의 전역 z-index. 🔴 **0 이상이어야 한다.** 폴리곤 기본값(-200000)처럼
  /// 음수면 네이버 지도의 **심벌(지명·POI 라벨) 아래**에 그려져, 안개가 지명을 가리지
  /// 못한다. 마커(200000)·내 위치(300000)보다는 아래라 스팟·발자취 마커와 내 위치 점은
  /// 안개 위에 보인다 — 안 보이면 갈 곳을 못 찾는다.
