          ...
          children: [
            ...
            const _Section(
              ...
              body:
                  ...
                  '「위치공유」를 켠 경우 30분마다, 앱 실행 중 약 15m 이동할 때마다\n'
                  '• 사진 — 방문 인증 사진, 발자취에 첨부한 사진 — 이용자가 촬영·첨부할 때\n'
                  '• 이용기록 — 방문 인증 이력(장소·시각·좌표), 작성한 글귀, 좋아요, 친구(동행) 요청·수락 이력\n'
                  '• 메시지 — 친구와 주고받은 메시지 내용, 보낸 시각 — 친구에게 메시지를 보낼 때\n'
                  '• 알림 토큰 — 기기의 푸시 알림 식별자(FCM 토큰) — 알림을 끄지 않은 경우\n'
            ...
            const _Section(
              ...
              body:
                  ...
                  '• 여행 성향 — 성향이 유사한 친구(동행) 추천\n'
                  '• 메시지 — 친구 요청이 수락된 두 이용자 사이의 대화 전달·표시\n'
                  '• 알림 토큰 — 친구 요청·수락과 새 메시지를 기기로 알리는 데만 사용합니다. 알림에는 '
                  '보낸 사람의 닉네임까지만 담기며 메시지 내용은 담지 않습니다. 위치를 기준으로 한 '
                  '알림(스팟 근처 도착 등)은 보내지 않습니다.\n'
            ...
            const _Section(
              ...
              body:
                  '• 앱의 설정 화면에서 직접 회원 탈퇴할 수 있으며, 탈퇴하는 즉시 계정과 '
                  '관련 정보(방문 인증 기록·사진, 발자취와 좋아요, 친구(동행) 요청·수락 이력, 주고받은 메시지, '
                  '성향 결과, 걸어온 자리)를 파기합니다. 되돌릴 수 없습니다.\n'
                  '• 앱을 이용할 수 없는 상황이라면 11장의 연락처로 요청해 주셔도 됩니다.\n'
                  '• 이용자가 작성한 발자취는 앱에서 직접 삭제할 수 있으며, 삭제 시 즉시 파기됩니다.\n'
                  '• 친구를 끊으면 그 친구와 주고받은 메시지 전체가 양쪽 모두에게서 즉시 파기됩니다. '
                  '어느 한쪽이 탈퇴해도 같습니다.\n'
                  '• 알림 토큰은 앱의 설정에서 알림을 끄면 즉시 삭제됩니다. 로그아웃·탈퇴 때도 삭제됩니다.\n'
            ...
            const _Section(
              ...
              body:
                  ...
                  '• 친구(동행) 추천 대상이 되었을 때의 닉네임·여행 성향 유형\n'
                  '• 친구에게 보낸 메시지 내용·보낸 시각 — 그 친구에게만 보이며, 친구 요청이 수락되기 전에는 '
                  '메시지를 보낼 수 없습니다.\n'
                  ...
                  '유지되고 이전 위치는 갱신 시 덮어써집니다. 기본값은 꺼짐이며 설정 화면에서 '
                  '언제든 켜고 끌 수 있습니다.',
            ),
            const _Section(
              title: '6. 개인정보 처리의 위탁',
            ...
            const _Section(
              ...
              body:
                  ...
                  '• 회원 탈퇴 및 개인정보 삭제 — 앱의 설정 화면에서 직접 수행 '
                  '(연락처로 요청하셔도 됩니다)',
            ),
            const _Section(
            ...
            const _Section(
              ...
              body: '본 방침을 변경할 경우 시행일 및 변경 내용을 앱 또는 공개 페이지에 공지합니다.\n\n'
                  '• 2026년 9월 18일 — 푸시 알림 기능 추가에 따라 수집 항목(알림 토큰)·이용 목적·보유 기간을 추가\n'
