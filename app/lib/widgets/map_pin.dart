...
class _HumanFootprintPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final sole = Path()
      ..moveTo(p(-0.21, 0.38).dx, p(-0.21, 0.38).dy)
      ..cubicTo(
          p(-0.19, 0.24).dx, p(-0.19, 0.24).dy, p(0.25, 0.22).dx, p(0.25, 0.22).dy, p(0.24, 0.42).dx, p(0.24, 0.42).dy)
      ..cubicTo(
          p(0.23, 0.53).dx, p(0.23, 0.53).dy, p(0.06, 0.57).dx, p(0.06, 0.57).dy, p(0.08, 0.67).dx, p(0.08, 0.67).dy)
      ..cubicTo(
          p(0.10, 0.75).dx, p(0.10, 0.75).dy, p(0.17, 0.79).dx, p(0.17, 0.79).dy, p(0.14, 0.88).dx, p(0.14, 0.88).dy)
      ..cubicTo(p(0.09, 0.97).dx, p(0.09, 0.97).dy, p(-0.13, 0.97).dx, p(-0.13, 0.97).dy, p(-0.15, 0.84).dx,
          p(-0.15, 0.84).dy)
      ..cubicTo(p(-0.18, 0.70).dx, p(-0.18, 0.70).dy, p(-0.26, 0.52).dx, p(-0.26, 0.52).dy, p(-0.21, 0.38).dx,
          p(-0.21, 0.38).dy)
      ..close();
    // 발바닥 안도 채운다(시진, 09-15) — 윤곽선만 두면 핀 위에서 속이 비쳐 보여 흐릿하다.
    // 발가락과는 띄운다 — 붙으면 한 덩어리로 읽힌다(시진, 09-15).
    ...
    const toes = <(double x, double y, double rx, double ry)>[
      (0.19, 0.09, 0.050, 0.065),
      (0.06, 0.06, 0.042, 0.052),
      (-0.06, 0.09, 0.037, 0.047),
      (-0.15, 0.15, 0.032, 0.042),
      (-0.23, 0.23, 0.028, 0.037),
    ];
