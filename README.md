...
- 지역별 탐험률(예: `경기도 3%`)을 실시간 수치로 피드백하여 즉각적인 성취감 제공 — 화면에는 **시/도 단위**로 보이고, 「탐험 현황」에서 전국 지도와 시/도 배지로 펼쳐집니다
...
- **익명 · 30분 텀 · 스팟 기준** — 누구인지 드러내지 않고, 초 단위로 따라다니지 않으며, 좌표가 아니라 "어떤 스팟 근처"만 전달합니다. 실명·실시간·정밀 좌표는 감시에 가깝고 여행자에게 필요한 정보도 아니기 때문입니다
...
> 인증이 동작합니다 — 발급 절차는 [docs/FIREBASE_AUTH_SETUP.md](docs/FIREBASE_AUTH_SETUP.md) 참고.
> 서비스 계정 키는 ⛔ 커밋 금지이므로 PM(@PGH0621)에게 비공개로 받으세요.
...
> ⚠️ **`--dart-define` 두 개를 넣어야 제대로 뜹니다.**
>
> ```bash
> flutter run \
>   --dart-define=NAVER_MAP_CLIENT_ID=발급받은_ID \
>   --dart-define=API_BASE_URL=https://fogapp.taild500a1.ts.net
> ```
>
> | 필요한 것 | 상태 |
> |---|---|
> | `google-services.json` · `firebase_options.dart` | ✅ 커밋돼 있음 ([#2](../../issues/2) 종료, 정책은 [#42](../../pull/42)) |
> | `NAVER_MAP_CLIENT_ID` | ⚠️ **미주입 시 지도가 렌더링되지 않습니다** |
> | `API_BASE_URL` | 기본값 `http://10.0.2.2:8080`(에뮬레이터 → 호스트). 실기기면 배포 주소를 넣으세요 |
>
> 📱 **그냥 폰에 넣어보려면** 빌드 없이 `https://fogapp.taild500a1.ts.net/fogapp.apk` 를 안드로이드 브라우저에서 열면 바로 설치됩니다.
...
| `GET` | `/api/conquest` | 지역별 탐험률 (**시/군/구 단위** 집계 — 앱이 `aggregateBySido` 로 시/도로 합산해 보여준다) | 3 |

스팟 데이터는 `SpotCollectionRunner`(관광공사 OpenAPI 수집 배치)가 `spots.content_id` 업서트로 적재합니다.

---

## 📈 개발 현황 (2026-09-15) — `v0.4.0` · ✅ **9/14 출품 마감 — 제출 결과는 [#158](../../issues/158)**

**기능은 마감 전에 완성됐습니다.** 스팟에 도달해 사진을 찍으면 안개가 걷히고, 스팟 정보가 해금되며, 지역 정복률이 올라갑니다. 길목에 남긴 글귀가 지도에 뜨고 탭하면 읽히며, 성향이 맞는 동행을 추천받아 요청·수락하는 것까지 됩니다.

| | |
|---|---|
| **배포 서버** | `https://fogapp.taild500a1.ts.net` (네이버 클라우드 + Tailscale Funnel · HTTPS) |
| **헬스체크** | `/api/health` → `{"status":"UP"}` |
| **APK** | `/fogapp.apk` — 안드로이드 폰 브라우저에서 바로 설치 |
| **데이터** | **12,600 스팟 / 17개 시도** · 소개글 2,866 |
| **출품 스토어** | **원스토어 단독** — [출시 가이드](docs/ONESTORE_RELEASE.md) |

> ⏸ **서버는 지금 정지해 뒀습니다.** 클라우드 이전은 끝났고([#154](../../issues/154)), 비용 절감을 위해
> 꺼둔 상태입니다. **스토어 등록·심사 직전에 켭니다** — 그때까지 위 주소는 응답하지 않습니다.

| Phase | 상태 |
|-------|------|
| **0~5** 기반·인증·지도·탐험 루프·발자취·매칭 | ✅ 완료 |
| **5.5** 발자취 재설계 | ✅ **완료** — 5건 전부 |
| **6** 실시간 위치 공유 | ✅ **6-1(여정)·6-3(익명 위치 공유) 병합 완료** — [#192](../../pull/192)·[#201](../../pull/201). 6-4(FCM)는 미착수, 6-5는 취소 |
| **7** 완성 & 출품 | 🔴 **스토어 등록·발표 자료가 남았습니다** |

`main` 은 `v0.4.0`, `dev` 에 Phase 5.5 완결분이 쌓여 있습니다. 다음 태그는 `v0.5.0` 이고 **스토어에 올릴 빌드가 여기서 나옵니다.**

단계별 상세와 남은 일정은 [planning.md](planning.md)를 보세요.

### 스팟 수집 배치 — 이미 돌았습니다

배포 DB 에는 **12,600 스팟 / 17개 시도**가 적재돼 있습니다([#155](../../issues/155)). 아래는 **로컬에서 처음부터 채울 때만** 필요합니다.

> 📌 소개글은 **2,866건**뿐입니다. 상세조회가 스팟 1건당 API 1회라 전국을 다 채우려면 일일 한도로
> 11일 이상 걸립니다. **소개글은 해금 후에만 보이므로** 지금은 이대로 두고, 필요할 때 이어서 채웁니다.
...
> **일일 호출 한도가 있습니다** (개발계정 기준 약 1,000회). 소진되면 이렇게 뜨고 그 실행은 즉시 멈춥니다.
>
> ```
> 관광공사 API 일일 호출 한도를 소진해 N건을 남기고 중단했습니다.
> 한도는 매일 초기화되므로 내일 다시 실행하면 이어서 채웁니다.
> ```
>
> 스팟이 수천 건이면 **며칠에 걸쳐 나눠 채우게 됩니다.** 마커는 목록 수집만으로 바로 뜨니, 소개글이 덜 찼다고 시연이 막히지는 않습니다.
