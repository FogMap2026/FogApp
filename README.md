...
| [docs/IOS_INSTALL.md](docs/IOS_INSTALL.md) | **iOS 실기기 설치** — 무서명 `.ipa` 를 무료 Apple ID 로 서명해 넣기 |
...
`dev`는 [PR #105](../../pull/105)로 `main`에 승격돼 **`v0.2.0`** 태그가 붙어 있습니다. 두 브랜치는 같은 상태입니다.

> 🔴 **실행 전 반드시 확인**: `spots` 테이블이 **비어 있습니다.** 그대로 띄우면 **지도에 마커가 없고
> 정복률이 `--%`** 로 나옵니다 — **버그가 아닙니다.** 관광공사 OpenAPI 키를 넣고 아래 절차를 한 번
> 실행해야 합니다. (9/3 첫 실행에서 나온 버그 3건은 [#107](../../pull/107)로 해소됐습니다.)
