...
> 🔴 **실행 전 반드시 확인**: `spots` 테이블이 **비어 있습니다.** 수집 배치를 아직 아무도 한 번도
> 돌리지 못했습니다 — 그대로 띄우면 **지도에 마커가 없고 정복률이 `--%`** 로 나옵니다. **버그가 아닙니다.**
> 관광공사 OpenAPI 키를 넣고 아래 절차를 한 번 실행해야 합니다.
