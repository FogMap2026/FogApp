...
>   --dart-define=API_BASE_URL=https://desktop-i6a90dk.taild500a1.ts.net
> ```
>
> | 필요한 것 | 상태 |
> |---|---|
...
>
> 📱 **그냥 폰에 넣어보려면** 빌드 없이 `https://desktop-i6a90dk.taild500a1.ts.net/fogapp.apk` 를 안드로이드 브라우저에서 열면 바로 설치됩니다.
...
| | |
|---|---|
| **배포 서버** | `https://desktop-i6a90dk.taild500a1.ts.net` (Tailscale Funnel · HTTPS) |
| **헬스체크** | `/api/health` → `{"status":"UP"}` |
| **APK** | `/fogapp.apk` — 안드로이드 폰 브라우저에서 바로 설치 |
| **데이터** | **2,669 스팟 / 2,664 소개글** (서울·부산·제주) |
| **출품 스토어** | **원스토어 단독** — [출시 가이드](docs/ONESTORE_RELEASE.md) |

> ⚠️ **서버는 팀원 PC 한 대(@songkh1201)에서 돕니다.** 상시 가동 환경으로 옮기는 것이 마감 전 필수 작업입니다.
...
배포 DB 에는 **2,669 스팟 / 2,664 소개글**이 적재돼 있습니다. 아래는 **로컬에서 처음부터 채울 때만** 필요합니다.
